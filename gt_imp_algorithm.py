...
from gt_base_algorithm import gt_base
...
def price_score_imp(task_dict, worker_dict, t, v, worker_list,w_skill, related_tasks):
    avg_task_cost = 0
    # result = w_skill // 10
    cur_task_cost = 0
...
def task_utility(ti, task_dict,_assigned_task,task_pre_assign, worker_dict, group, v, w_list, w_skill, alpha, worker_v, max_s,max_p,TC,FT):
    ...
    profit_w = price_score_imp(task_dict, worker_dict, ti, v, w_list,w_skill, related_tasks)

    cur_total=alpha * (total_score) / max_s + (1 - alpha) * (profit_w / max_p)
    # related_tasks.remove(ti)
    # related_total=0
    # for a in related_tasks:
    #     if a in _assigned_task:
    #         if len(task_pre_assign[a]['list'])==len(task_dict[a]['Kt']):
    #             w_list=task_pre_assign[a]['list']
    #             w_score=0
    #             for w in w_list:
    #                 w_score += worker_dict[w]['score']
    #             w_score=w_score/len(w_list)
    #             w_profit = price_score_imp1(task_dict, worker_dict, a, v, w_list)
    #             related_total+=alpha * (w_score) / max_s + (1 - alpha) * (w_profit / max_p)
...
def gt_imp_algorithm(task_dict, worker_dict, max_time, move_cost, alpha, max_s, max_p, worker_v,reachable_dis):
    # performance
    full_assign = 0
    worker_assign_time={}
